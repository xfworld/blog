...
如果你有兴趣, 可以直接用 codex 基于本文思路改进 pg 代码: [《白嫖的 Codex 真香!》](../202603/20260317_05.md)  
...
graph TD
    A["tuplesort.c"]
    A --> A1["BumpContext / AllocSet <br/> for tuplecontext <br/> (unbounded vs bounded)"]
    A --> A2["MemoryContextReset(tuplecontext) <br/> after each run dump"]
    A --> A3["MemoryContextResetOnly(tuplecontext) <br/> + pfree(memtuples) <br/> at merge start"]
    A --> A4["Slab allocator with free-list <br/> for merge phase tuples"]
    A --> A5["lastReturnedTuple lazy <br/> free on next gettuple call"]
    A --> A6["LogicalTapeClose on exhausted <br/> tapes during on-the-fly merge"]
    A --> A7["free_sort_tuple for <br/> bounded heap evictions"]
    A --> A8["tuplesort_reset: reuse <br/> maincontext across batches"]
```
  
```mermaid
graph TD
    B["nodeHash.c"]
    B --> B1["batchCxt reset per batch <br/> via ExecHashTableReset"]
    B --> B2["ExecHashIncreaseNumBatches: <br/> chunk-by-chunk eviction to disk"]
    B --> B3["ExecHashRemoveNextSkewBucket: <br/> early skew memory reclaim"]
    B --> B4["dense_alloc chunks: <br/> bulk-free via batchCxt reset"]
``` 
  
```mermaid
graph TD
    C["nodeAgg.c"]
    C --> C1["ResetExprContext(tmpcontext) <br/> per-input-tuple"]
    C --> C2["ReScanExprContext(aggcontexts) <br/> per-group boundary"]
    C --> C3["hash_agg_enter_spill_mode: <br/> caps memory by spilling to disk"]
    C --> C4["ReScanExprContext(hashcontext) <br/> + ResetTupleHashTable <br/> between spilled batches"]
    C --> C5["Selective column spilling <br/> (only needed columns)"]
    C --> C6["BumpContext for hash_tuplescxt: <br/> reset, not per-tuple pfree"]
```
